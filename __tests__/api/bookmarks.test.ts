// __tests__/api/bookmarks.test.ts
// Test suite for Lesson Bookmarks API
// Feature ID: feat-223

import { describe, it, expect, jest, beforeEach } from "@jest/globals";

const mockJson = jest.fn();
jest.mock("next/server", () => ({
  NextRequest: jest.fn(),
  NextResponse: { json: mockJson },
}));

const mockSupabase = {
  auth: {
    getUser: jest.fn(),
  },
  from: jest.fn(() => mockSupabase),
  select: jest.fn(() => mockSupabase),
  insert: jest.fn(() => mockSupabase),
  delete: jest.fn(() => mockSupabase),
  eq: jest.fn(() => mockSupabase),
  order: jest.fn(() => mockSupabase),
  single: jest.fn(),
};

jest.mock("@/lib/supabase/server", () => ({
  supabaseServer: () => mockSupabase,
}));

// Mock Request class for tests
class MockRequest {
  public nextUrl: { searchParams: URLSearchParams };
  public method: string;
  private body: any;

  constructor(url: string, init?: { method?: string; body?: string }) {
    const urlObj = new URL(url);
    this.nextUrl = { searchParams: urlObj.searchParams };
    this.method = init?.method || "GET";
    this.body = init?.body;
  }

  async json() {
    return JSON.parse(this.body);
  }
}

global.Request = MockRequest as any;

describe("Bookmarks API - feat-223", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.resetModules();
    mockJson.mockImplementation((data) => ({
      json: async () => data,
      status: 200,
    }));
  });

  describe("GET /api/bookmarks", () => {
    it("should return 401 if user is not authenticated", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({ data: { user: null } });
      mockJson.mockReturnValue({
        json: async () => ({ error: "Unauthorized" }),
        status: 401,
      });

      const { GET } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks");

      const response = await GET(req as any);
      expect(mockJson).toHaveBeenCalledWith(
        { error: "Unauthorized" },
        { status: 401 }
      );
    });

    it("should check if specific lesson is bookmarked", async () => {
      const mockBookmark = {
        id: "bookmark-123",
        user_id: "user-123",
        lesson_id: "lesson-123",
      };

      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockSelect = jest.fn().mockReturnThis();
      const mockEq = jest.fn().mockReturnThis();
      const mockSingle = jest
        .fn()
        .mockResolvedValue({ data: mockBookmark, error: null });

      mockSupabase.from.mockReturnValue({
        select: mockSelect,
        eq: mockEq,
        single: mockSingle,
      });

      mockSelect.mockReturnValue({
        eq: mockEq,
      });

      mockEq.mockReturnValue({
        eq: mockEq,
        single: mockSingle,
      });

      const { GET } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks?lessonId=lesson-123");
      const response = await GET(req as any);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.bookmarked).toBe(true);
      expect(mockSupabase.from).toHaveBeenCalledWith("lesson_bookmarks");
    });

    it("should return false if lesson is not bookmarked", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockSelect = jest.fn().mockReturnThis();
      const mockEq = jest.fn().mockReturnThis();
      const mockSingle = jest
        .fn()
        .mockResolvedValue({ data: null, error: { code: "PGRST116" } });

      mockSupabase.from.mockReturnValue({
        select: mockSelect,
        eq: mockEq,
        single: mockSingle,
      });

      mockSelect.mockReturnValue({
        eq: mockEq,
      });

      mockEq.mockReturnValue({
        eq: mockEq,
        single: mockSingle,
      });

      const { GET } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks?lessonId=lesson-123");
      const response = await GET(req as any);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.bookmarked).toBe(false);
    });

    it("should fetch all bookmarks with lesson details", async () => {
      const mockBookmarks = [
        {
          id: "bookmark-1",
          lesson_id: "lesson-1",
          created_at: new Date().toISOString(),
          lessons: {
            id: "lesson-1",
            title: "Test Lesson",
            modules: {
              title: "Test Module",
              courses: { id: "course-1", title: "Test Course", slug: "test-course" },
            },
          },
        },
      ];

      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockSelect = jest.fn().mockReturnThis();
      const mockEq = jest.fn().mockReturnThis();
      const mockOrder = jest.fn().mockResolvedValue({ data: mockBookmarks, error: null });

      mockSupabase.from.mockReturnValue({
        select: mockSelect,
        eq: mockEq,
        order: mockOrder,
      });

      mockSelect.mockReturnValue({
        eq: mockEq,
      });

      mockEq.mockReturnValue({
        order: mockOrder,
      });

      const { GET } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks");
      const response = await GET(req as any);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.bookmarks).toEqual(mockBookmarks);
      expect(mockOrder).toHaveBeenCalledWith("created_at", { ascending: false });
    });
  });

  describe("POST /api/bookmarks", () => {
    it("should return 401 if user is not authenticated", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({ data: { user: null } });

      const { POST } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks", {
        method: "POST",
        body: JSON.stringify({ lessonId: "lesson-123" }),
      });

      const response = await POST(req as any);
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe("Unauthorized");
    });

    it("should return 400 if lessonId is missing", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const { POST } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks", {
        method: "POST",
        body: JSON.stringify({}),
      });

      const response = await POST(req as any);
      const data = await response.json();

      expect(response.status).toBe(400);
      expect(data.error).toBe("lessonId required");
    });

    it("should create bookmark for authenticated user", async () => {
      const mockBookmark = {
        id: "bookmark-123",
        user_id: "user-123",
        lesson_id: "lesson-123",
        created_at: new Date().toISOString(),
      };

      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockInsert = jest.fn().mockReturnThis();
      const mockSelect = jest.fn().mockReturnThis();
      const mockSingle = jest
        .fn()
        .mockResolvedValue({ data: mockBookmark, error: null });

      mockSupabase.from.mockReturnValue({
        insert: mockInsert,
        select: mockSelect,
        single: mockSingle,
      });

      mockInsert.mockReturnValue({
        select: mockSelect,
      });

      mockSelect.mockReturnValue({
        single: mockSingle,
      });

      const { POST } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks", {
        method: "POST",
        body: JSON.stringify({ lessonId: "lesson-123" }),
      });

      const response = await POST(req as any);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.bookmark).toEqual(mockBookmark);
      expect(mockSupabase.from).toHaveBeenCalledWith("lesson_bookmarks");
      expect(mockInsert).toHaveBeenCalledWith({
        user_id: "user-123",
        lesson_id: "lesson-123",
      });
    });

    it("should return 409 if lesson already bookmarked", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockInsert = jest.fn().mockReturnThis();
      const mockSelect = jest.fn().mockReturnThis();
      const mockSingle = jest.fn().mockResolvedValue({
        data: null,
        error: { code: "23505", message: "duplicate key value" },
      });

      mockSupabase.from.mockReturnValue({
        insert: mockInsert,
        select: mockSelect,
        single: mockSingle,
      });

      mockInsert.mockReturnValue({
        select: mockSelect,
      });

      mockSelect.mockReturnValue({
        single: mockSingle,
      });

      const { POST } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks", {
        method: "POST",
        body: JSON.stringify({ lessonId: "lesson-123" }),
      });

      const response = await POST(req as any);
      const data = await response.json();

      expect(response.status).toBe(409);
      expect(data.error).toBe("Already bookmarked");
    });
  });

  describe("DELETE /api/bookmarks", () => {
    it("should return 401 if user is not authenticated", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({ data: { user: null } });

      const { DELETE } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks?lessonId=123", {
        method: "DELETE",
      });

      const response = await DELETE(req as any);
      const data = await response.json();

      expect(response.status).toBe(401);
      expect(data.error).toBe("Unauthorized");
    });

    it("should return 400 if lessonId is missing", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const { DELETE } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks", {
        method: "DELETE",
      });

      const response = await DELETE(req as any);
      const data = await response.json();

      expect(response.status).toBe(400);
      expect(data.error).toBe("lessonId required");
    });

    it("should delete bookmark for authenticated user", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockDelete = jest.fn().mockReturnThis();
      const mockEq = jest.fn();

      const eqChain = {
        eq: jest.fn().mockResolvedValue({ error: null }),
      };

      mockEq.mockReturnValue(eqChain);

      mockSupabase.from.mockReturnValue({
        delete: mockDelete,
      });

      mockDelete.mockReturnValue({
        eq: mockEq,
      });

      const { DELETE } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks?lessonId=lesson-123", {
        method: "DELETE",
      });

      const response = await DELETE(req as any);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.success).toBe(true);
      expect(mockSupabase.from).toHaveBeenCalledWith("lesson_bookmarks");
      expect(mockDelete).toHaveBeenCalled();
    });

    it("should return 500 if delete fails", async () => {
      mockSupabase.auth.getUser.mockResolvedValue({
        data: { user: { id: "user-123" } },
      });

      const mockDelete = jest.fn().mockReturnThis();
      const mockEq = jest.fn();

      const eqChain = {
        eq: jest.fn().mockResolvedValue({ error: { message: "Database error" } }),
      };

      mockEq.mockReturnValue(eqChain);

      mockSupabase.from.mockReturnValue({
        delete: mockDelete,
      });

      mockDelete.mockReturnValue({
        eq: mockEq,
      });

      const { DELETE } = await import("@/app/api/bookmarks/route");
      const req = new Request("http://localhost/api/bookmarks?lessonId=lesson-123", {
        method: "DELETE",
      });

      const response = await DELETE(req as any);
      const data = await response.json();

      expect(response.status).toBe(500);
      expect(data.error).toBe("Database error");
    });
  });
});
